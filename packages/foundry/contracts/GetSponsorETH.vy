# @version ^0.3.9
"""
@title getSponsorETH main contract based on snekmate's modern and Gas-Efficient ERC-1155 Implementation
@custom:contract-name getSponsorETH
@license GNU Affero General Public License v3.0 only
@author luksgrin
@custom:libauthor pcaversaccio
@custom:libauthor jtriley.eth
...
import SponsoredPools as ISponsoredPools
...
struct sponsorship_details:
    id: uint256
    start_time: uint256
    time_to_expiry: uint256
    pledge: String[280] # 280 characters is the maximum length of a tweet for reference
    is_perpetual: bool

MAX_TIME_TO_EXPIRY: constant(uint256) = 365*24*60*60 # 365 days in seconds

ownerOf: public(HashMap[uint256, address])
sponsoredPools: public(HashMap[uint256, address])
_sponsorships: HashMap[uint256, sponsorship_details]
isAllowedToken: public(HashMap[address, bool])
min_amount_fund: HashMap[address, uint256]
a_tokens: HashMap[address, address]
_counter: uint256
_sponsored_bools_blueprint: immutable(address)
...
event TokenAllowanceUpdate:
    token: address
    isAllowed: bool

event StakeWithdrawn:
    sponsorshipId: indexed(uint256)
    token: indexed(address)
    staked: indexed(address)
...
def __init__(
    ...
    _weth: address,
    _aweth: address,
    __sponsored_bools_blueprint: address
    ):
    """
    ...
    # ERC1155 Constructor body ###############
    _BASE_URI = "SPONSORETH"                 #
                                             #
    ...
    self.weth = _weth
    self.isAllowedToken[_weth] = True
    self.a_tokens[_weth] = _aweth
    IWETH(self.weth).approve(self.lendingPool, max_value(uint256))
    _sponsored_bools_blueprint = __sponsored_bools_blueprint

@external
@view
def sponsorships(idx: uint256) -> (uint256, uint256, uint256, String[280], bool):
    return (
        self._sponsorships[idx].id,
        self._sponsorships[idx].start_time,
        self._sponsorships[idx].time_to_expiry,
        self._sponsorships[idx].pledge,
        self._sponsorships[idx].is_perpetual
    )
...
def createSponsor(
    ...
    pledge: String[280],
    is_perpetual: bool,
    configs: DynArray[String[280], 10] # Will have to look into this
    ):

    self.ownerOf[self._counter] = msg.sender
    log NewSponsor(self._counter, msg.sender, pledge)

    details: sponsorship_details = sponsorship_details({
        id: self._counter,
    ...
    self._sponsorships[self._counter] = details

    sp: ISponsoredPools = ISponsoredPools(
        create_from_blueprint(
            _sponsored_bools_blueprint,
            code_offset=3
        )
    )
    sp.iinit(self.lendingPool, self, msg.sender)
    self.sponsoredPools[self._counter] = sp.address

    self._set_configs(self._counter, configs)
...
def fund(
    ...
    # assert self._is_not_expired(sponsorship_id), "expired"

    token_: address = token
    amount_: uint256 = amount

    if token_ != empty(address):
        self._fund_with_token(sponsorship_id, token_, is_staking, amount_)
    else:
        if is_staking:
            amount_ = msg.value
            IWETH(self.weth).deposit(value=msg.value)
            token_ = self.weth

            assert self.isAllowedToken[token_], "!allowed token"
            assert amount_ >= self.min_amount_fund[token_], "small amount"
            # get sponsored pool
            sp: ISponsoredPools = ISponsoredPools(
                self.sponsoredPools[sponsorship_id]
            )

            ILendingPool(self.lendingPool).deposit(
                token,
                amount,
                sp.address,
                0
            ...
            raw_call(
                self.ownerOf[sponsorship_id],
                b"",
                value=msg.value
            )

    self._safe_mint(msg.sender, sponsorship_id, 1, b"")

    log Fund(sponsorship_id, token_, msg.sender, is_staking, user, message)

    # Present in the original solidity contract. Why?
    self._safe_mint(msg.sender, sponsorship_id, 1, b"")

@external
def withdrawStake(sponsorship_id: uint256, token: address):
    sp: ISponsoredPools = ISponsoredPools(
        self.sponsoredPools[sponsorship_id]
    )
    sp.unstake(msg.sender, token)
    log StakeWithdrawn(sponsorship_id, token, msg.sender)

@external
def claim(sponsorship_id: uint256, token: address):
    sp: ISponsoredPools = ISponsoredPools(
        self.sponsoredPools[sponsorship_id]
    )
    a_token: address = self.a_tokens[token]
    sp.claim(token, a_token)
    log Claimed(sponsorship_id, token)

@external
def setConfigs(
    sponsor_id: uint256,
    configs: DynArray[String[280], 10]
    ):
    self._set_configs(sponsor_id, configs)
...
def _set_configs(
    ...
    assert len(configs) % 2 == 0, "Invalid configs val"
    assert self.ownerOf[sponsor_id] == msg.sender, "not allowed"

    for i in range(0, 25):
        log Config(sponsor_id, configs[2*i], configs[2*i + 1])
        if i == len(configs)/2 - 1:
            break
...
def updateAllowed(
    ...
    assert self.isAllowedToken[token] != is_allowed, "!update"
    self.isAllowedToken[token] = is_allowed

    if is_allowed:
        self.min_amount_fund[token] = min_amount
        IERC20(token).approve(
            self.lendingPool,
            max_value(uint256)
        )
        self.a_tokens[token] = a_token
    else:
        self.min_amount_fund[token] = 0
        IERC20(token).approve(
            self.lendingPool,
            0
        )

    log TokenAllowanceUpdate(token, is_allowed)
...
def _fund(
    ...
    owner_: address = self.ownerOf[sponsorship_id]
    assert owner_ != empty(address), "Sponsor not found"
    assert IERC20(token).transferFrom(sender, owner_, amount), "Transfer failed"

@internal
def _fund_with_staking(
    sponsorship_id: uint256,
    sender: address,
    token: address,
    amount: uint256
    ):
    self._fund(sponsorship_id, sender, token, amount)
...
@internal
def _fund_with_token(
    sponsorship_id: uint256,
    token: address,
    is_staking: bool,
    amount: uint256
    ):
    assert self.isAllowedToken[token], "!allowed token"
    assert amount >= self.min_amount_fund[token], "small amount"
    if is_staking:
        self._fund_with_staking(sponsorship_id, msg.sender, token, amount)
    else:
        self._fund(sponsorship_id, msg.sender, token, amount)

@internal
@view
def _is_not_expired(sponsorship_id: uint256) -> bool:
    details: sponsorship_details = self._sponsorships[sponsorship_id]
    is_expired: bool = details.is_perpetual or (
        (details.start_time + details.time_to_expiry) > block.timestamp
    )
    return is_expired
    
@external
@view
def getClaim(sponsorship_id: uint256, token: address) -> uint256:
    # get sponsored pool
    sp: ISponsoredPools = ISponsoredPools(
        self.sponsoredPools[sponsorship_id]
    )
    a_token: address = self.a_tokens[token]
    return sp.claimable(token, a_token)
...
#####################################################
#        snekmate's ERC-1155 Implementation         #
#####################################################
...
import lib.snekmate.src.tokens.interfaces.IERC1155 as IERC1155
implements: IERC1155
...
# syntax.
import lib.snekmate.src.tokens.interfaces.IERC1155MetadataURI as IERC1155MetadataURI
implements: IERC1155MetadataURI


# @dev We import the `IERC1155Receiver` interface, which
# is written using standard Vyper syntax.
import lib.snekmate.src.tokens.interfaces.IERC1155Receiver as IERC1155Receiver
...
event TransferBatch:
    ...
    owner: indexed(address)
    to: indexed(address)
    ids: DynArray[uint256, 255]
    amounts: DynArray[uint256, 255]
...
@external
def safeBatchTransferFrom(owner: address, to: address, ids: DynArray[uint256, 255], amounts: DynArray[uint256, 255],
                          data: Bytes[1_024]):
    """
...
@external
@view
def balanceOfBatch(owners: DynArray[address, 255], ids: DynArray[uint256, 255]) -> DynArray[uint256, 255]:
    """
    ...
    batch_balances: DynArray[uint256, 255] = []
    ...
    for owner in owners:
        ...
        # parameter `255`, which is bounded by the
...
@external
def burn_batch(owner: address, ids: DynArray[uint256, 255], amounts: DynArray[uint256, 255]):
    """
...
@external
def safe_mint_batch(owner: address, ids: DynArray[uint256, 255], amounts: DynArray[uint256, 255], data: Bytes[1_024]):
    """
...
@internal
def _safe_batch_transfer_from(owner: address, to: address, ids: DynArray[uint256, 255], amounts: DynArray[uint256, 255],
                              data: Bytes[1_024]):
    """
...
@internal
def _safe_mint_batch(owner: address, ids: DynArray[uint256, 255], amounts: DynArray[uint256, 255], data: Bytes[1_024]):
    """
...
@internal
def _burn_batch(owner: address, ids: DynArray[uint256, 255], amounts: DynArray[uint256, 255]):
    """
...
@internal
def _check_on_erc1155_batch_received(owner: address, to: address, ids: DynArray[uint256, 255], amounts: DynArray[uint256, 255],
...
@internal
def _before_token_transfer(owner: address, to: address, ids: DynArray[uint256, 255], amounts: DynArray[uint256, 255],
                           data: Bytes[1_024]):
    """
...
@internal
def _after_token_transfer(owner: address, to: address, ids: DynArray[uint256, 255], amounts: DynArray[uint256, 255],
                          data: Bytes[1_024]):
    """
